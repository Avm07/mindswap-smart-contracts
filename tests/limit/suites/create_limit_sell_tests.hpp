...
try {
    ...
    WASM_ASSERT("SYS@everipediaiq deposit is not opened",
				limit.create_limit_sell(N(alice), N(alice), sys_token_asset, iq_token_asset));
...
BOOST_FIXTURE_TEST_CASE(create_full_fill_deal_limit_sell_test, limit_tester)
try {
    extended_asset sys_token_asset{asset::from_string("1.000 SYS"), N(everipediaiq)};
	extended_asset iq_token_asset{asset::from_string("100.000 IQ"), N(everipediaiq)};
...
BOOST_FIXTURE_TEST_CASE(create_half_fill_deal_limit_sell_test, limit_tester)
try {
    ...
    auto sell_order = limit.get_sell_order(0, 1001);
    BOOST_REQUIRE_EQUAL(true, sell_order.is_null());
...
BOOST_FIXTURE_TEST_CASE(create_over_fill_deal_limit_sell_test, limit_tester)
try {
