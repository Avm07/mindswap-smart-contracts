...
BOOST_AUTO_TEST_SUITE(fill_buy_order_tests)

BOOST_FIXTURE_TEST_CASE(asserts_fill_buy_order_test, limit_tester)
try {
	ERROR("missing authority of mindswaparbi",
          limit.fill_buy_order(N(alice), 0, 1000));

    WASM_ASSERT("fill_buy_order: order is not exist",
                 limit.fill_buy_order(N(mindswaparbi), 0, 999));
}
FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE(fill_buy_order_test, limit_tester)
try {
    ...
    SUCCESS(limit.fill_buy_order(N(mindswaparbi), 0, 1000));
