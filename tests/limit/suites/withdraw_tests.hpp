...
try {
	...
	WASM_ASSERT("withdraw: quantity should be positive", limit.withdraw(N(alice), N(alice), N(carol), negative_iq_token_asset, ""));
...
try {
	...
	REQUIRE_MATCHING_OBJECT(alice_deposit,
							mvo()("id", 0)("contract", "everipediaiq")("balance", "0.000 IQ")("balance_in_orders", "0.000 IQ"));

	auto alice_balance = iq.get_account(N(alice), "3,IQ");

	REQUIRE_MATCHING_OBJECT(alice_balance, mvo()("balance", "100.000 IQ"));
}
FC_LOG_AND_RETHROW()
