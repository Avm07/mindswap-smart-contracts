...
try {
    WASM_ASSERT("arbitrage_order_trade: invalid order type",
                arb.arbitrage_order_trade(N(alice), 1, N(invalid), 1000, symbol::from_string("3,SYSIQ").to_symbol_code()));

    WASM_ASSERT("arbitrage_order_trade: invalid market id",
                arb.arbitrage_order_trade(N(alice), 1000, N(buy), 1000, symbol::from_string("3,SYSIQ").to_symbol_code()));
            
    WASM_ASSERT("arbitrage_order_trade: invalid order id",
                arb.arbitrage_order_trade(N(alice), 0, N(buy), 999, symbol::from_string("3,SYSIQ").to_symbol_code()));

    WASM_ASSERT("arbitrage_order_trade: mindswap pool is not exist",
                arb.arbitrage_order_trade(N(alice), 0, N(sell), 1000, symbol::from_string("3,NOEXIST").to_symbol_code()));
}
FC_LOG_AND_RETHROW()
