...
action_result arbitrage_test_api::arbitrage_order_trade(const name& signer, const name& account, const uint64_t& market_id, const name& order_type, const uint64_t& order_id,  const asset& amount, const symbol_code& mindswap_pool)
{
	return push_action(signer, contract, N(arbordtrade), mvo()("account", account)("market_id", market_id)("order_type", order_type)("order_id", order_id)("amount", amount)("mindswap_pool", mindswap_pool));
}
 
action_result arbitrage_test_api::arbitrage_pair_trade(const name& signer, const name& account, const uint64_t& market_id, const name& orders_type, const std::vector<uint64_t>& orders_ids,  const symbol_code& mindswap_pool)
{
